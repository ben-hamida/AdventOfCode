...
static bool IsFurthestIn(Amphipod amphipod) => amphipod.X == 2;
...
{
    bool IsOccupied(int x, int y) => amphipods.Any(a => a.Y == y && a.X == x);
    Amphipod GetAtPosition(int x, int y) => amphipods.Single(a => a.Y == y && a.X == x);

    if (IsInARoom(amphipod))
    {
        if (IsFurthestIn(amphipod) && IsOccupied(x: 1, y: amphipod.Y))
        {
        ...
        {
            if (IsFurthestIn(amphipod))
            {
                // It's furthest in the target room, no reason to ever move
                yield break;
            }

            if (GetAtPosition(x: 2, y: destinationY).Type == amphipod.Type)
            {
                // It's in the closest to the hall and the amphipod furthest in is of the same type, no need to move
                yield break;
            }
        ...
        var possibleHallPositions = new int[] { 0, 1, 3, 5, 7, 9, 10 }
    ...
    else
    {
        ...
        if (amphipodsAtDestination.Length == 2 ||
            (amphipodsAtDestination.Length == 1 && amphipodsAtDestination[0].Type != amphipod.Type))
        {
            yield break;
        }
        ...
        int destinationX = amphipodsAtDestination.Any() ? 1 : 2;
