...
int numberOfSuccessfulInitialVelocities = 0;

for (int xVelocity = 1; xVelocity < 287; xVelocity++)
{
    for (int yVelocity = -102; yVelocity < 1000; yVelocity++)
    {
        bool hit = SimulateLaunch((xVelocity, yVelocity));
        if (hit)
        {
            numberOfSuccessfulInitialVelocities++;
        }
    }
}

Console.WriteLine(numberOfSuccessfulInitialVelocities);

static bool SimulateLaunch((int x, int y) initialVelocity)
{
