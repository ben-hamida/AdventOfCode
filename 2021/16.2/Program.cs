using System.Text;

const string hex = "";

var hexToBinaryMap = new Dictionary<char, string>
{
    { '0', "0000"},
    { '1', "0001"},
    { '2', "0010"}, 
    { '3', "0011"},
    { '4', "0100"}, 
    { '5', "0101"}, 
    { '6', "0110"}, 
    { '7', "0111"},
    { '8', "1000"}, 
    { '9', "1001"}, 
    { 'A', "1010"}, 
    { 'B', "1011"},
    { 'C', "1100"}, 
    { 'D', "1101"}, 
    { 'E', "1110"}, 
    { 'F', "1111"}
};

string binary = string.Concat(hex.Select(h => hexToBinaryMap[h]));

Packet packet = ParsePacket(binary);
Console.WriteLine(packet.Value);

static Packet ParsePacket(string packet)
{
    string versionString = packet[..3];
    int version = Convert.ToInt32(versionString, 2);

    string typeIdString = packet.Substring(3, 3);
    int typeId = Convert.ToInt32(typeIdString, 2);

    int packetLength = 6;

    if (typeId == 4)
    {
        int i = 0;

        StringBuilder sb = new();
        while (true)
        {
            sb.Append(packet.AsSpan(i + packetLength + 1, 4));
            if (packet[i + packetLength] == '0') // Last group
            {
                i += 5;
                break;
            }

            i += 5;
        }

        long value = Convert.ToInt64(sb.ToString(), 2);
        packetLength += i;
        return new Packet(version, packetLength, value);
    }

    List<Packet> subPackets = new();
    char lengthType = packet[packetLength];
    packetLength++;
    if (lengthType == '0')
    {
        string lengthString = packet.Substring(packetLength, 15);
        int subPacketsLength = Convert.ToInt32(lengthString, 2);
        packetLength += 15;

        int processedSubPacketsLength = 0;
        while (processedSubPacketsLength < subPacketsLength)
        {
            Packet subPacket = ParsePacket(packet.Substring(packetLength));
            processedSubPacketsLength += subPacket.Length;
            packetLength += subPacket.Length;
            subPackets.Add(subPacket);
        }
    }
    else // lengthType == '1'
    {
        string numberOfSubPacketsString = packet.Substring(packetLength, 11);
        int numberOfSubPackets = Convert.ToInt32(numberOfSubPacketsString, 2);
        packetLength += 11;
        for (int i = 0; i < numberOfSubPackets; i++)
        {
            Packet subPacket = ParsePacket(packet.Substring(packetLength));
            packetLength += subPacket.Length;
            subPackets.Add(subPacket);
        }
    }

    checked
    {
        return typeId switch
        {
            0 => new Packet(version, packetLength, subPackets.Sum(p => p.Value)),
            1 => new Packet(version, packetLength, subPackets.Select(p => p.Value).Aggregate((x, y) => x * y)),
            2 => new Packet(version, packetLength, subPackets.MinBy(p => p.Value)!.Value),
            3 => new Packet(version, packetLength, subPackets.MaxBy(p => p.Value)!.Value),
            5 => new Packet(version, packetLength, subPackets[0].Value > subPackets[1].Value ? 1 : 0),
            6 => new Packet(version, packetLength, subPackets[0].Value < subPackets[1].Value ? 1 : 0),
            7 => new Packet(version, packetLength, subPackets[0].Value == subPackets[1].Value ? 1 : 0),
            _ => throw new InvalidOperationException()
        };
    }
}

internal record Packet(int Version, int Length, long Value);